...
%AttackTypes = (
	...
	my $Owner = ($npc->GetOwnerID > 0 ? $entity_list->GetClientByID($npc->GetOwnerID()) : ($npc->GetSwarmOwner() > 0 ? $entity_list->GetClientByID($npc->GetSwarmOwner()) : 0));
	...
	if($EnableRemoving) {
			...
			if ($PrimaryModel) {
				if($Owner->HasTwoHanderEquipped()) { $PrimSkill = 3; $npc->SetSecSkill(28);} else { $PrimSkill = 0; }
			...
			if($SecondaryModel) {
				if($Owner->HasShieldEquiped()) { $SecSkill = 28; } else { $SecSkill = 0; }
		...
		if ($PrimaryModel) {
			quest::wearchange(7, $PrimaryModel);
			if($Owner->HasTwoHanderEquipped()) { $PrimSkill = 3; $npc->SetSecSkill(28);} else { $PrimSkill = 1; }
			$npc->SetPrimSkill($PrimSkill);
		}
		if ($SecondaryModel) {
			quest::wearchange(8, $SecondaryModel);
			if($Owner->HasShieldEquiped()) { $SecSkill = 28; } else { $SecSkill = 1; }
			$npc->SetSecSkill($SecSkill);
		}
	}
}