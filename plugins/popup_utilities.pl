# plugin::Popup($title, $text, $button_type, $popup_id);
# plugin::Popup($title, $text, $button_type, $button_one_id, $button_two_id, $button_one_text, $button_two_text);
sub Popup {
	...
	if (!$title) {
		$title = quest::getnpcnamebyid($npc->GetNPCTypeID()) . " says,";
	}
	$title =~ s/\_/ /g;
	if ($button_type == 2) {
		my (
		...
		$client->Popup2($title, plugin::PWColor($text, "Orange"), $button_one_id, $button_two_id, 1, 600, $button_one_text, $button_two_text);
	} else {
		my $popup_id = shift;
		quest::popup($title, plugin::PWColor($text, "Orange"), $popup_id, $button_type);
	}
}
...
sub YesNo {
	...
	return (
		$bool ?
		plugin::PWColor("Yes", "Green") :
		plugin::PWColor("No", "Red1")
	);
}
...
sub HandlePopup {
	...
	if ($popupid ~~ [501..510]) {
		plugin::CreateInstance("Group", ($popupid - 500), $client->GetMoney(3, 0), $client->GetMoney(3, 0));
	} elsif ($popupid == 10000) {
		if ($instanceid == 0) {
			quest::movepc(381, 61.57, -0.15, 4.88, 387.3);
		} else {
			quest::MovePCInstance(381, $instanceid, 61.57, -0.15, 4.88, 387.3);
		}
	} elsif ($popupid ~~ [25,26]) {
		if($popupid == 25) {
			my @donator_categories = plugin::Donator_Shop_Types();
			plugin::Message("You can view the following categories:");
			foreach $category (@donator_categories) {
				plugin::Message("- " . quest::saylink("shop $category",1,"$category") . "");
			}
		} else {
			my $link = plugin::PWHyperLink("https://www.paypal.com/myaccount/transfer/send","Paypal - Send Money");
			my $email = 'admin@eqinfinity.com';
			my $discord = '@ Splose#5349';
			#my $address = Email::Valid->address($email);
			plugin::Popup("Payment Information",
			"Donator coins are 1:100. Meaning that 100 donator coins are equivalent to 1 USD. <br> <br>
			
			• Go to: $link <br>
			• Send to: $email <br>
			• Send as Friends & Family <br>
			• In the 'Notes' section put your Character Name <br>
			• (Optional) - Include your Discord name to be added to the Donator channel <br>
			• PM me directly on Discord $discord <br> <br>
			Thank you for playing on Rise of Norrath, an EQEmpires server!",
			0);
		}
	} elsif ($popupid == 27) {	#:: Waist of Time De-level Confirmation
		plugin::Delevel($client);
	}
}
...
sub PWAutoCenter{
	...
	my $Result = "";
	while ($n < $IC){ $Result .= "&nbsp;"; $n++; }
	if(!$SendCharLength){ return "$Result $Cent"; my $CentLength = 0; }
	elsif($SendCharLength == 2){ return "$Result"; }
	else{ return "There are $IC characters in this phrase, use this arguement for Option 2 in Arguement 2"; }
